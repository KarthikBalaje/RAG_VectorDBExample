...
def retrieve(question: str, k: int = TOP_K) -> list[dict]:
    ...
    doc_ids = results["ids"][0] if results.get("ids") and isinstance(results["ids"], list) and len(results["ids"]) > 0 else []
    texts = results["documents"][0] if results.get("documents") and isinstance(results["documents"], list) and len(results["documents"]) > 0 else []
    metas = results["metadatas"][0] if results.get("metadatas") and isinstance(results["metadatas"], list) and len(results["metadatas"]) > 0 else []
    dists = results["distances"][0] if results.get("distances") and isinstance(results["distances"], list) and len(results["distances"]) > 0 else []
    
    for doc_id, text, meta, dist in zip(doc_ids, texts, metas, dists):
        chunks.append({
            "id":       doc_id,
...
def ask_with_rag(question: str, chunks: list[dict]) -> str:
    ...
    context_block = "\n\n".join(
        f"[Source: {c['metadata'].get('title', 'Unknown')} — {c['metadata'].get('source', 'N/A')}, {c['metadata'].get('year', 'N/A')}]\n{c['text']}"
        for c in chunks
    )
    system_prompt = (
...
DEMO_QUESTIONS = [
    "What are the key features of electric vehicles and their advantages?",
    "How do autonomous vehicles navigate and make driving decisions?",
    "What are the main types of bikes and their specific use cases?",
]
